...
if ($fdb eq 1) {
    print BOLD RED "\nSorry, the project name $PRJ was already choosen :(\nBut, no fear just choose another one ;)\n";
    die RESET, "\n";
    } else {
...
####################################
#ERASE ALL TABLES IF fdb IS SET TO 0
####################################
sub cleanTBL {
   # get total number of arguments passed.
   my $tbl = $_[0];
   print BOLD RED "Killing all the previous data on $tbl\n";
  
  $dbh->do("
  truncate EXP_$PRJ\_$tbl
  ");
  $dbh->commit();
}
...
print BOLD UNDERLINE REVERSE WHITE ON_YELLOW BLINK "   Done\n   Have a nice day \;)   \n";
