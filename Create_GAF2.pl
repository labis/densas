#!/usr/bin/perl -w
use strict;
use DBI;
use DateTime;

# CONFIG VARIABLES
my $st_time = time(); # set the starting time for calculation
my $platform = "mysql";
my $database = "midgutomics";
my $host = "143.106.4.249";
my $port = "3306";
my $user = "annotate";
my $pw = "b10ine0!";
#my $fstFILE   = $ARGV[0];
    my $usage = "You must use this as follow:\n perl Create_GAF.pl [PROJ NAME] [TAXID]\n";
    my $PRJ = shift or die $usage;
    my $taxID = shift or die $usage;
    my ($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime();
    #All variables
    my $COL1 = "MidgutDB";
    my $COL2 = '';
    my $COL3 = '';
    my $COL4 = "";
    my $COL5 = "";
    my $COL6 = "DB:MidgutDB";
    my $COL7 = "IEA";
    my $COL8 = "";
    my $COL9 = "";
    my $COL10 = "";
    my $COL11 = "";
    my $COL12 = "Transcript";
    my $COL13 = "taxon:$taxID";
    my $COL14 = DateTime->now(time_zone => "local")->ymd('');
    my $COL15 = "LABIS";
    my $COL16 = "";
    my $COL17 = "";


#Conects to the MySQL database
my $dbh = DBI->connect("dbi:mysql:$database:$host:$port", "$user", "$pw",
                    { RaiseError => 1, AutoCommit => 0 });

###########################
#CREATE TABLE EXISTS PRJ_Desc
###########################

print "Building a home for the DESCRIPTION search\n";

$dbh->do("CREATE TABLE IF NOT EXISTS  `$PRJ\_DESC` (
  descID int(11) NOT NULL AUTO_INCREMENT,
  Seqname varchar(150) DEFAULT NULL,
  `Desc` text DEFAULT NULL,
  PRIMARY KEY (descID)
)
ENGINE = INNODB
AUTO_INCREMENT = 1
AVG_ROW_LENGTH = 111
CHARACTER SET utf8
COLLATE utf8_general_ci;
");
  $dbh->commit();

###########################
#CREATE TABLE EXISTS PRJ_ANN
###########################

print "Building a home for the ANNOTATION search\n";

$dbh->do("CREATE TABLE IF NOT EXISTS `$PRJ\_ANN` (
  ANN_ID int(11) NOT NULL AUTO_INCREMENT,
  SeqName varchar(50) DEFAULT NULL,
  GOID varchar(50) DEFAULT NULL,
  Aspect varchar(50) DEFAULT NULL,
  GO_DESC varchar(250) DEFAULT NULL,
  PRIMARY KEY (ANN_ID),
  INDEX GO_ASPECT (Aspect),
  INDEX GO_ID (GOID),
  INDEX SeqName (SeqName)
)
ENGINE = INNODB
AUTO_INCREMENT = 1
AVG_ROW_LENGTH = 104
CHARACTER SET utf8
COLLATE utf8_general_ci;
");
  $dbh->commit();


  my $CREATEGAF = $dbh->selectall_arrayref("

SELECT
  $PRJ\_DESC.Seqname,
  $PRJ\_DESC.`Desc`,
  $PRJ\_ANN.GOID,
  CASE WHEN $PRJ\_ANN.Aspect = 'B' THEN 'P' WHEN $PRJ\_ANN.Aspect = 'M' THEN 'F' WHEN $PRJ\_ANN.Aspect = 'C' THEN 'C' END AS category
FROM $PRJ\_ANN
  INNER JOIN $PRJ\_DESC
    ON $PRJ\_ANN.Seqname = $PRJ\_DESC.Seqname   
")
  or die "print unable to connect to the DB";

open(my $fh, '>', $PRJ.".gaf"); # Open file to write
  #Start creating the file
  print $fh "!gaf-version: 2.0\n";
foreach my $row (@$CREATEGAF) {
     my ($COL2, $COL10, $COL5, $COL9) = @$row;
     $COL3 = $COL2;
     print $fh "$COL1\t$COL2\t$COL3\t$COL4\t$COL5\t$COL6\t$COL7\t$COL8\t$COL9\t$COL10\t$COL11\t$COL12\t$COL13\t$COL14\t$COL15\t$COL16\t$COL17\n";
     }
close $fh; # close file to write
    my $endtime = time();
    my $runtime = $endtime - $st_time;
    print "The file $PRJ\.gaf was created in $runtime seconds.\nNice job!\n"
